...
from taki_ai.layers import *  # noqa
...
class DeepConvNet:
    ...
    def __init__(self, input_dim=(3, 28, 28),
                 conv_param_1={'filter_num': 48,
                               'filter_size': 3, 'pad': 1, 'stride': 1},
                 conv_param_2={'filter_num': 48,
                               'filter_size': 3, 'pad': 1, 'stride': 1},
                 conv_param_3={'filter_num': 96,
                               'filter_size': 3, 'pad': 1, 'stride': 1},
                 conv_param_4={'filter_num': 96,
                               'filter_size': 3, 'pad': 2, 'stride': 1},
                 conv_param_5={'filter_num': 192,
                               'filter_size': 3, 'pad': 1, 'stride': 1},
                 conv_param_6={'filter_num': 192,
                               'filter_size': 3, 'pad': 1, 'stride': 1},
                 hidden_size=50, output_size=2):
        # 重みの初期化===========
        # 各層のニューロンひとつあたりが、前層のニューロンといくつのつながりがあるか（TODO:自動で計算する）
        pre_node_nums = np.array(
            [3*3*3, 48*3*3, 48*3*3, 96*3*3, 96*3*3, 192*3*3, 192*4*4, hidden_size])
        ...
        self.params['W7'] = weight_init_scales[6] * \
            np.random.randn(192*4*4, hidden_size)
    ...
    def output(self, x):
        tmp = self.predict(x)
        y = self.last_layer.forward_output_layer(tmp)
        return y
